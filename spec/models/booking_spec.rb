...
RSpec.describe Booking, type: :model do
  ...
  describe "#no_show!" do
    ...
    context "inside the standard no show window" do
      let(:lesson) { create(:lesson, duration_in_minutes: 120, starts_at: Time.now - (4.days + 90.minutes) ) }
      let(:booking) { create(:booking, status: "pending", lesson: lesson) }
      ...
      it "does not create a NoShowCharge" do
        expect {
          subject
        }.to change { NoShowCharge.count }.by(1)
      end
    end
    ...
    context "lesson happened before the 5th of the current month an in the previous year" do
      before { Timecop.freeze(Date.new(2017,1,6)) }
      after { Timecop.return }

      let(:lesson) { create(:lesson, starts_at: Date.new(2016,12,29).to_time) }
      let(:booking) { create(:booking, status: "completed", lesson: lesson) }
