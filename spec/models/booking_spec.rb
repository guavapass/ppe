...
RSpec.describe Booking, type: :model do
  ...
  describe "#no_show!" do
    ...
    context "lesson happened within the current payout window" do
      before { Timecop.freeze(Date.new(2016,10,20)) }
      after { Timecop.return }

      let(:lesson) { create(:lesson, starts_at: Date.new(2016,10,18).to_time) }
      let(:booking) { create(:booking, status: "completed", lesson: lesson) }

      it "does not creates a billing adjustment" do
        expect {
          subject
        }.not_to change { BillingAdjustment.count }
      end
    end

    context "lesson happened before the 4th of the current month and is within the no show window" do
    ...
    # context "lesson happened within the payout window but after the end of the month" do
    #   before { Timecop.freeze(Date.new(2016,10,3)) }
    #   after { Timecop.return }

    #   let(:lesson) { create(:lesson, starts_at: Date.new(2016,10,1).to_time) }
    #   let(:booking) { create(:booking, status: "completed", lesson: lesson) }
